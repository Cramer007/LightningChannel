...
const CLN_RPC_PATH: &str = "/Users/portablegaucher/.lightning/testnet4/lightning-rpc";
...
fn print_usage() {
    ...
    eprintln!("  lnurl-client request-withdraw <url|ip> ");
}
...
fn channel_request(url: &Url) -> Result<()> {
    ...
    if resp.tag != "channelRequest" {
        return Err(anyhow!("Unexpected tag: {}", resp.tag));
    }
...
fn withdraw_request(url: &Url) -> Result<()> {
    ...
    println!("  Callback: {}", resp.callback);
    println!("  k1: {}", resp.k1);
    println!("  Description: {}", resp.defaultDescription);
    println!("  Min withdrawable: {}", resp.minWithdrawable);
    println!("  Max withdrawable: {}", resp.maxWithdrawable);
