...
async fn open_channel(
    ...
    // Check if k1 exists in HashSet and consume it
    let mut k1_store = state.k1_store.lock().await;
    let k1_valid = k1_store.remove(&params.k1);
    drop(k1_store);
...
async fn withdraw(
    ...
    let request = PayRequest {
        ...
        amount_msat: None, // invoice fixe le montant
        label: None,
        riskfactor: None,
        maxfeepercent: None,
        retry_for: None,
    ...
    match client.call(cln_rpc::Request::Pay(request)).await {
        ...
        _ => (
            ...
            Json(WithdrawResponse {
                status: "ERROR".to_string(),
                reason: Some("Unexpected response".to_string()),
            }),
        ),
    }
}
...
#[tokio::main]
async fn main() {
    let client = match cln_rpc::ClnRpc::new("/Users/portablegaucher/.lightning/testnet4/lightning-rpc").await {
        Ok(c) => c,
        Err(e) => {
